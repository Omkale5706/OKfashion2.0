...
function mapBackendRecommendations(analysisData: any) {
  const backend = analysisData?.recommendations
  if (!Array.isArray(backend) || backend.length === 0) return []

  return backend
    .filter((rec: any) => rec.type === "outfit")
    .map((rec: any, index: number) => ({
      id: Number(rec.id || index + 1),
      name: rec.title,
      items: Array.isArray(rec.items) ? rec.items.map((i: any) => `${i.type} - ${i.brand}`) : [rec.description],
      colors: rec.colors || [],
      occasion: rec.category || "Personalized",
      confidence: Math.round((rec.confidence || 0.85) * 100),
      image: rec.image || "/casual-chic-outfit.jpg",
      source: "backend",
    }))
}
...
export function OutfitRecommendations({ analysisData, userImage }: OutfitRecommendationsProps) {
  ...
  const outfits = useMemo(() => {
    const backendOutfits = mapBackendRecommendations(analysisData)
    return backendOutfits.length > 0 ? backendOutfits : getPersonalizedOutfits(analysisData)
  }, [analysisData])
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {outfit.colors.map((color: string, index: number) => (
